...
description: Use when running inference probes on the mesh, routing tasks to pi or
  hermes harnesses, or configuring model loading across LM Studio (adeck:1234), vLLM
  (zrrh:8000), or llama-server (zrrh direct). Covers gateway config, ctx ceiling procedure,
  tool-call probing, KV quant config, dense vs MoE tradeoffs, and harness architecture.
compatibility: Designed for the daemonturgy mesh (nxiz/zrrh/adeck). Requires nix develop
  flake shell and Tailscale mesh access.
metadata:
  author: zk
  version: '1.0'
  category: inference
