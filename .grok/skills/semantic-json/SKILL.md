---
name: semantic-json
description: Use when working with Obsidian Canvas files as cognitive modeling tools
  — compiling spatial/visual structure into deterministic JSON, designing canvas layouts
  with semantic color and edge conventions, exporting structured data from canvas,
  or importing JSON into canvas form.
metadata:
  author: zk
  version: '1.0'
  category: obsidian
