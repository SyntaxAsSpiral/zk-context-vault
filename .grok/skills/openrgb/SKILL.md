---
name: openrgb
description: Advanced OpenRGB integration for ambient computing — SDK server automation,
  remote display surfaces, presence indicators, notification channels, and event-driven
  effects using openrgb-python. Use when building RGB as an ambient interface layer,
  presence/notification systems, multi-host lighting coordination, or system-event-driven
  effects.
compatibility: Requires OpenRGB with SDK server enabled, openrgb-python (pip). Network
  features require hosts reachable over TCP. Linux recommended; partial Windows/macOS
  support.
metadata:
  version: '0.2'
  author: zk
