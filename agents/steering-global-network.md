...
| zrrh | 100.126.60.24 | NixOS 26.05 (Yarara) | Couch PC / Local Inference + Media/Gaming | online |
...
NFS over Tailscale IPs provides cross-device filesystem access. Managed declaratively via `modules/taildrive.nix` in the nix-os flake. All mounts use systemd automount (lazy mount-on-access, 60s idle timeout, no boot dependency).

**Local disks:**
- `nxiz`: `/mnt/repository` (btrfs, ssd) — context vault and dev repos; `/mnt/archive` (btrfs) — archive storage
- `zrrh`: `/mnt/media` (btrfs, ssd) — media library; `/mnt/games` (btrfs, ssd) — game storage
- `adeck`: `/mnt/vault` (btrfs, zstd) — backup vault

**NFS exports:**
- `nxiz` exports `/mnt/repository` to adeck + zrrh
- `zrrh` exports `/mnt/media` to nxiz (rw) + adeck (ro)
- `adeck` exports `/mnt/vault` to nxiz + zrrh

**NFS mounts (automount):**
- `nxiz` mounts: `zrrh:/mnt/media` → `/mnt/media`, `adeck:/mnt/vault` → `/mnt/vault`
- `adeck` mounts: `nxiz:/mnt/repository` → `/mnt/repository`, `zrrh:/mnt/media` → `/mnt/media`
- `zrrh` mounts: `nxiz:/mnt/repository` → `/mnt/repository`, `adeck:/mnt/vault` → `/mnt/vault`
