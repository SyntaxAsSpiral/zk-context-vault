...
### 🧊 Protected Paths
**yamas:** `docs/**` is edit-on-request only — no edits unless operator explicitly requests. Content there may drift or be incomplete; do not "fix" or normalize. This constraint applies exclusively to `docs/**`, not to other documentation files (AGENTS.md, README.md, steering files, etc.).
**niyamas:** —
